...
meetup16 = Meetup.create!(title: "Game all night, sleep all day", description: "My place is a little tight but maybe we could snuggle", location: "1306 Rue Bélanger, Montreal, QC H2G 1A1", start_time: Time.now, end_time: Time.now + 148*60*60, date: Time.now, user:martin, game:Game.find(Game.first.id + 1))
...
meetup9 = Meetup.create!(title: "super SMASH tournament ;)", description: "My place is a little tight but maybe we could snuggle", location: "7244 Rue Hutchison, Montreal, QC H3N 1Z1", start_time: Time.now + 32*60*60, end_time: Time.now + 35*60*60, date: Time.now, user:ali, game:Game.find(Game.first.id + 20))
