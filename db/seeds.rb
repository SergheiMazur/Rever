...
meetup14 = Meetup.create!(title: "FIFA, Rum and Poker $100 min", description: "Everyone welcome, BYOB!", location: "2022 Rue Centre, Montreal, QC H3K 1J3", start_time: Time.now, end_time: Time.now + 154*60*60, date: Time.now, user:aude, game:Game.find(Game.first.id + 7))
meetup16 = Meetup.create!(title: "Game all night, sleep all day", description: "My place is a little tight but maybe we could snuggle", location: "1306 Rue Bélanger, Montreal, QC H2G 1A1", start_time: Time.now, end_time: Time.now + 148*60*60, date: Time.now, user:konstantin, game:Game.find(Game.first.id + 1))
meetup4 = Meetup.create!(title: "Let's pretend it's the 90s :)", description: "My place is a little tight but maybe we could snuggle", location: "2285 Rue Saint-Mathieu, Montreal, QC H3H 2J7", start_time: Time.now, end_time: Time.now + 1*60*60, date: Time.now, user:sara, game:Game.find(Game.first.id + 4))
...
meetup7 = Meetup.create!(title: "Pokemon Stadium Tournament", description: "I have a packed fridge full of food :)", location: "1305 Avenue Papineau, Montreal, QC H2K 4H3", start_time: Time.now, end_time: Time.now + 1*60*60, date: Time.now, user:celine, game:Game.find(Game.first.id + 19))
...
meetup9 = Meetup.create!(title: "SMASH tournament!", description: "My place is a little tight but maybe we could snuggle", location: "7244 Rue Hutchison, Montreal, QC H3N 1Z1", start_time: Time.now + 32*60*60, end_time: Time.now + 35*60*60, date: Time.now + 24*60*60, user:martin, game:Game.find(Game.first.id + 20))
...
meetup10 = Meetup.create!(title: "*Need for Speed* Loser gets Dinner.", description: "Looking to play tournament style", location: "4433 Boulevard Saint-Laurent, Montreal, QC H2W 1Z8", start_time: Time.now + 72*60*60, end_time: Time.now + 75*60*60, date: Time.now, user:michael, game:Game.find(Game.first.id + 11))
meetup18 = Meetup.create!(title: "Weekly bro time", description: "I'll be streaming I Love Lucy episodes", location: "509 Avenue Atwater, Montreal, QC H3J 2J6", start_time: Time.now + 72*60*60, end_time: Time.now + 72*60*60, date: Time.now, user:justin, game:Game.find(Game.first.id + 5))
...
meetup13 = Meetup.create!(title: "Pushup tournament + game nite", description: "I have a packed fridge full of food :)", location: "7661 Rue Saint-Denis, Montreal, QC H2R 2E7", start_time: Time.now, end_time: Time.now + 109*60*60, date: Time.now, user:guillaume, game:Game.find(Game.first.id + 12))
...
meetup3 = Meetup.create!(title: "Tournois de FIFA 18", description: "Come join my fornight office party (we have a keg!) :)", location: "Coopérative d'habitation du Châtelet, 2063 Rue Saint-Denis, Montreal, QC H2X 3K8", start_time: Time.now, end_time: Time.now + 1*60*60, date: Time.now, user:mg, game:Game.find(Game.first.id + 7))
meetup20 = Meetup.create!(title: "Rever and CHILL?", description: "Everyone welcome, BYOB!", location: "1115 Rue Sherbrooke O, Montreal, QC H3A 1H3", start_time: Time.now, end_time: Time.now + 154*60*60, date: Time.now, user:aaron, game:Game.find(Game.first.id + 6))
meetup15 = Meetup.create!(title: "Retro night at my house ☮", description: "Everyone welcome, BYOB!", location: "5184 Rue Beaubien E, Montreal, QC H1T 1W1", start_time: Time.now, end_time: Time.now + 1*60*60, date: Time.now, user:sawyer, game:Game.find(Game.first.id + 3))
meetup2 = Meetup.create!(title: "Zelda lovers unite", description: "My place only for 3 persons", location: "812 Avenue du Mont-Royal E, Montreal, QC H2J 1W8", start_time: Time.now + 2*60*60, end_time: Time.now + 4*60*60, date: Time.now + 2*60*60, user:furkan, game:Game.find(Game.first.id + 9))
...
Guest.create!(current_status:"Pending", user:martin, meetup:meetup9)
