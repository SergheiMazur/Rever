...
meetup5 = Meetup.create!(title: "Weekly 2k tournament (Congrats @h00plyf for victory)", description: "Looking to play tournament style", location: "2027 Rue Saint Timothee, Montreal, H2L 3P5", start_time: Time.now + 72*60*60, end_time: Time.now + 75*60*60, date: Time.now, user:elie, game:Game.find(Game.first.id + 8))
...
meetup18 = Meetup.create!(title: "Weekly bro time", description: "I'll be streaming I Love Lucy episodes", location: "3415 McTavish Street, Montreal, Quebec H3A 0C8", start_time: Time.now + 72*60*60, end_time: Time.now + 72*60*60, date: Time.now, user:justin, game:Game.find(Game.first.id + 22))
...
meetup13 = Meetup.create!(title: "Pushup tournament + games", description: "I have a packed fridge full of food :)", location: "5333 Casgrain Ave, Montreal, QC H2T 1X3", start_time: Time.now, end_time: Time.now + 109*60*60, date: Time.now, user:guillaume, game:Game.find(Game.first.id + 12))
...
meetup2 = Meetup.create!(title: "Kingdom Hearts battle", description: "My place only for 3 persons", location: "LMVR-Le Laurier, 286 Avenue Laurier O, Montreal, QC H2V 2K2", start_time: Time.now + 2*60*60, end_time: Time.now + 4*60*60, date: Time.now + 2*60*60, user:furkan, game:Game.find(Game.first.id + 3))
